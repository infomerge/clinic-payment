...
class CLSYSTEM{
    ...
    function getPaymentData(){
        ...
        if($this->readFromAccDetail):
            $sql = "SELECT * FROM acc_result as a , acc_detail as b WHERE a.rid = b.rid AND a.targetym = '{$this->targetym}'";
        ...
        */
                ...
                if($k == $v['category']){
                  ...
                  if($k == 'Y'){
                    ...
                    if(isset($data[$v['original_pid']]['category']['F'])){
                        $data[$v['original_pid']]['category']['F'] += $v['tensu'];
                    }else{
                        $data[$v['original_pid']]['category']['F'] = $v['tensu'];
                    }
                    ...
                    if(isset($data[$v['original_pid']]['category']['O'])){
                        $data[$v['original_pid']]['category']['O'] += $v['tensu'];
                    }else{
                        $data[$v['original_pid']]['category']['O'] = $v['tensu'];
                    }   
                  ...
                  #「T」カテゴリとまだみぬ「P,Q,R,S,U,V,W,X,Z」は「O：病理診断 = 保険：その他」に合算
                    ...
                    if(isset($data[$v['original_pid']]['category'][$k])){
                        $data[$v['original_pid']]['category'][$k] += $v['tensu'];
                    }else{
                        $data[$v['original_pid']]['category'][$k] = $v['tensu'];
                    }   
                    ...
                    if(isset($data[$v['original_pid']]['total_tensu'])){
                        $data[$v['original_pid']]['total_tensu'] += $v['tensu'];
                    }else{
                        $data[$v['original_pid']]['total_tensu'] = $v['tensu'];
                    }
                }
        ...
        foreach ($data as $original_pid => $patient_data) {
            ...
            foreach($patient_data['srd'] as $iryo_srm => $v2){
                ...
                if(isset($m_max[$original_pid][$iryo_srm])){
    ...
    function generateRPdata(){
        ...
        foreach($data as $original_pid => $patient_data):
            ...
            $sql = "INSERT INTO acc_result (gid,rst,ap,ec,god,cod,am,tx,sf,ta,em,nm,original_pid,srm,targetym,reqid,rp_disableflag,rp_errorflag,rp_errormsg)
                    VALUES (0,0,0,0,0,'$inv_id','{$patient_data['total_copayment']}',0,0,0,'','','$original_pid',0,'{$this->targetym}',null,'{$patient_data['data']['direct_debit']}',0,'');";
            #echo $sql."\n";
...
#echo $html;exit;
            ...
            if($this->manageperiod_flag == 1):
                ...
                $seikyu_month = date('Y年m月',strtotime($this->srd_start)) ;
            endif;
    ...
    $html .= "<tr>
            <th class=\"color333 font16 border_rb\">".$patient_data['category']['M']['title']."</th>
            <th class=\"color333 border_rb\">".$patient_data['category']['N']['title']."</th>
            <th class=\"color333 border_rb\">".$patient_data['category']['O']['title']."</th>
        ...
        foreach($patient_data['srm'] as $kaigo_key => $kaigo_value){
            ...
            if($kaigo_key == "data"):
                ...
                foreach ($kaigo_value['sid'] as $meisai){
                    $copeyment = 10 * $patient_data['srm']['data']['tensu'] * $patient_data['srm']['data']['rate'] /100;
