...
*/
    ...
    function generatePDF(){
        ...
        $newpage_offset2 = 78;
        $newpage_offset3 = 20;
        // 1ページ目の明細エリア行数上限（介護・医療の合算カウントと併用）。孤立ヘッダ防止の遅延オープンと合わせて調整
        ...
        foreach ($data as $original_pid => $patient_data) {
            ...
            #医療保険：介護が使った行を引き継ぐ。表のオープンは「最初の日付行を出す直前」まで遅延し、
            #1ページ目に明細が1行も無いのに見出しだけ出るのを防ぐ（mPDFの page-break-inside: avoid との相性も改善）
            $prior_meisai_rows = $has_kaigo ? $row_count : 0;
            $row_count = $prior_meisai_rows;
    ...
    foreach($patient_data['srd'] as $kk => $vv){
        ...
        foreach($vv as $k => $v){
            ...
            if(!$iryo_table_open){
                if($global_count == 0){
                    $check_offset_open = $first_page_offset;
                }else{
                    $check_offset_open = $newpage_offset2;
                }
                # 見出し2行のあとに日付行を置けないなら、改ページしてから医療表を開始
                $pre_break_before_iryo = ($row_count + 2 >= $check_offset_open);
                if($pre_break_before_iryo){
                    $tmp = "<div style='page-break-before: always;'></div>";
                    $html .= $tmp;
                    $global_count++;
                    $iryo_p2_count++;
                ...
                if($has_kaigo && !$pre_break_before_iryo){
                    $html .= "<div class=\"meisai-section-gap\"></div>";
                }
            ...
            #1ページ目は first_page_offset（介護+医療の合算行数で判定）。2ページ目以降は newpage_offset2
            if($global_count == 0){
