...
<form method="post" action="recept-upload-result-im.php" enctype="multipart/form-data">
...
#print_r($array_uk);exit;    #内科にUKが存在しない
    }
    if ($data[0] == "IR") {
...
#print_r($array_ir);
    ...
    if ($data[0] == "RE") {
		...
		$srm = convertDate($data[3]);       #【要確認】convertDateの方法は歯科と同じか？
		$birth = convertDate($data[6]);     #【要確認】convertDateの方法は歯科と同じか？
        ...
        $array_re = array('レコード名'=>'レセプト共通レコード',
                       ...
                       #'診療開始日'=>$data[9],    #歯科と【違う】→内科に存在しない
                       #'転帰区分'=>$data[10],
                       '病棟区分コード'=>$data[9],    #歯科と【違う】11→9
                       '負担区分コード'=>$data[10],    #歯科と【違う】12→10
                       'レセプト特記事項'=>$data[11],    #歯科と【違う】13→11
                       '予備'=>$data[15],    #歯科と【違う】14→15
                       'カルテ番号'=>$data[13],    #歯科と【違う】15→13
                       '請求情報'=>$data[21],    #歯科と【違う】16→21
                       '予備'=>$data[15],       #歯科と【違う】17→15
                       #'未来院請求コード'=>$data[18],#歯科と【違う】→内科に存在しない
                       '検索番号'=>$data[18],    #歯科と【違う】19→18
                       #'記録条件仕様公表年月(GYYMM)'=>$data[20],#歯科と【違う】→内科に存在しない
                       #'請求情報2'=>$data[21],     #歯科と【違う】→内科に存在しない
                       '予備'=>$data[16],    #歯科と【違う】22→16
                       '予備'=>$data[17],    #歯科と【違う】23→17
                       '予備'=>$data[19],    #歯科と【違う】24→19
...
#################################################################
    ...
    if ($data[0] == "HO") {
        ...
        $array_ho = array('レコード名'=>'保険者レコード',
                       '保険者番号'=>$data[1],    #歯科と同じ
                       '被保険者記号'=>mb_convert_kana(mb_convert_encoding($data[2], "UTF-8", "SJIS"), 'kvrn'),    #歯科と同じ
                       '被保険者番号'=>mb_convert_kana(mb_convert_encoding($data[3], "UTF-8", "SJIS"), 'kvrn'),    #歯科と同じ
                       '診療実日数'=>$data[4],    #歯科と同じ
                       '合計点数'=>$data[5],      #歯科と同じ

                       '食事療養回数'=>$data[7],        #歯科と【違う】6→7
                       '食事療養合計金額'=>$data[8],    #歯科と【違う】7→8
                       '職務上の事由コード'=>$data[9],    #歯科と【違う】8→9
                       '証明書番号'=>$data[10],          #歯科と【違う】9→10
                       '医療保険金額(円)'=>$data[11],     #歯科と【違う】10→11
                       '減免区分コード'=>$data[12],     #歯科と【違う】11→12
                       '減額割合(%)'=>$data[13],       #歯科と【違う】12→13
                       '減額金額(円)'=>$data[14],      #歯科と【違う】13→14
                      );
        #print_r($array_ho);
        ...
        $payer = $array_ir['審査支払機関'];     #内科はUKがないのでIR
        $prefecture = $array_ir['都道府県'];
        $irkkcode = $array_ir['医療機関コード'];
        $irkkname = $array_ir['医療機関名'];
    ...
    if ($data[0] == "KO") {
        ...
        $array_ko = array('レコード名'=>'公費レコード',
                       ...
                       '公費'=>$data[6],    #歯科と同じ
                       '一部負担金'=>$data[7],    #歯科と同じ
                       '食事療養回数'=>$data[10],    #歯科と【違う】8→10
                       '食事療養合計金額'=>$data[11],    #歯科と【違う】9→11
                      );
        #print_r($array_ko);
    ...
    if ($data[0] == "SY") {
        ...
        $array_hs = array('レコード名'=>'傷病名部位レコード',
                       '診療開始日'=>$data[2],    #歯科と【違う】1→2
                       '転帰区分コード'=>$data[3],      #歯科と【違う】2→3
                       #'歯式（傷病名）'=>'-(表示省略)-',
                       '傷病名'=>$data[1],      #歯科と【違う】4→1
                       '修飾語コード'=>$data[4],    #歯科と【違う】5→4
                       '傷病名称'=>$data[5],        #歯科と【違う】6→5
                       #'併存傷病名数'=>$data[7],    #歯科と【違う】→内科に存在しない
                       #'病態移行コード'=>$data[8],     #歯科と【違う】→内科に存在しない
                       '主傷病コード'=>$data[6],        #歯科と【違う】9→6
                       #'コメントコード'=>$data[10],    #歯科と【違う】→内科に存在しない
                       '補足コメント'=>$data[7],       #歯科と【違う】11→7
                       #'歯式(補足コメント)'=>$data[12],    #歯科と【違う】→内科に存在しない
                      );
        #print_r($array_hs);
        ...
        if ($irkk_exists) {
            ...
            $sql = "UPDATE account_info
                SET irkkname = '$irkkname', tel = '$irkk_tel'
                WHERE original_irkkcode = '$irkk_exists'";
            $dbh->query($sql);
        } else {
        ...
        $original_irkkcode = $irkk_postal_code = $irkk_address = $irkk_tel = "";
        foreach ($stmt as $row) {
        ...
        if(isset($irkkcode) && $original_irkkcode . $irkk_postal_code . $irkk_address . $irkk_tel != ""){
            $sql = "UPDATE re_patient
    ...
    if ($data[0] == "SI") {
        ...
        #for ($j = 78 ; $j <= 108; $j++) {
        $si_offset = 12;
        for ($j = $si_offset + 1 ; $j <= $si_offset + 31; $j++) {
            if ( $data[$j] >= 1 ) {
                $sid += 1;
                $original_irkkcode = $original_pid = $pid = "";
                $dt = sprintf('%02d', $j - $si_offset);
                $srd = $srm . $dt;
                if ($data[1] != "") {
    ...
    if ($data[0] == "SI") {
        ...
        $upper = isset($syubetsu_copy[0]['upper']) ? $syubetsu_copy[0]['upper'] : "";
    ...
    if ($data[0] == "IY") {
        ...
        $iy_offset = 12;
        #for ($j = 8 ; $j <= 38; $j++) {
        for ($j = $iy_offset + 1 ; $j <= $iy_offset + 31; $j++) {
            if ( $data[$j] >= 1 ) {
                $sid += 1;
                $original_irkkcode = $original_pid = $pid = "";
                #$dt = sprintf('%02d', $j - 7);
                $dt = sprintf('%02d', $j - $iy_offset);
                $srd = $srm . $dt;
                if ($data[1] != "") {
                    $sql = "SELECT * FROM shikibetsu_code WHERE code = $data[1]";
                    $stmt = $dbh->query($sql);
    ...
    if ($data[0] == "TO") {
        ...
        switch($array_re['レセプト特記事項']){
            ...
            case "4041":
                ...
                break;
            default:
                $ratio = isset($syubetsu_copy[0]['ratio']) ? $syubetsu_copy[0]['ratio'] : 0;
                break;
        }
        $upper = isset($syubetsu_copy[0]['upper']) ? $syubetsu_copy[0]['upper'] : "";
        ...
        $to_offset = 16;
        #for ($j = 14 ; $j <= 44; $j++) {
        for ($j = $to_offset + 1 ; $j <= $to_offset + 31; $j++) {
            if ( $data[$j] >= 1 ) {
                $sid += 1;
                $original_irkkcode = $original_pid = $pid = "";
                #$dt = sprintf('%02d', $j - 13);
                $dt = sprintf('%02d', $j - $to_offset);
                $srd = $srm . $dt;
                if ($data[1] != "") {
                    $sql = "SELECT * FROM shikibetsu_code WHERE code = $data[1]";
                    $stmt = $dbh->query($sql);
