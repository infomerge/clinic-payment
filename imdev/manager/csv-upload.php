...
while ( ( $dt = fgetcsv ( $handle, 200) ) !== FALSE ) {
    ...
    #codが内科（頭の2文字がim）じゃないものはスキップ
    if (substr($cod, 0, 2) !== 'im') {
        // 先頭2文字が "im" の場合の処理
        echo $cod."：内科ではない<br>\n";
        continue;
    }
    ...
    if(count($data)){
        ...
        $sql = "UPDATE re_shinryo SET manageperiod_status = '$manageperiod_status' WHERE original_pid = '$original_pid' and manageperiod_targetym = '$manageperiod_targetym' ;";
        #$message .= $sql."\n";
        #$dbh->query($sql);
        echo $sql."<br>\n";
        ...
        $sql = "UPDATE rek_service SET manageperiod_status = '$manageperiod_status' WHERE original_pid = '$original_pid' and manageperiod_targetym = '$manageperiod_targetym' ;";
        #$message .= $sql."\n";
        #$dbh->query($sql);
        echo $sql."<br>\n";
        ...
        $sql = "UPDATE appendix SET manageperiod_status = '$manageperiod_status' WHERE original_pid = '$original_pid' and manageperiod_targetym = '$manageperiod_targetym' ;";
        #$message .= $sql."\n";
        #$dbh->query($sql);
        echo $sql."<br>\n";
