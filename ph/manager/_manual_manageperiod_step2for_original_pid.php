...
if (isset($GLOBALS['argv'][1])) {
  $original_pid = $GLOBALS['argv'][1];
}else{
  $original_pid = isset($_GET["original_pid"]) ? $_GET["original_pid"] : "";
}
if (isset($GLOBALS['argv'][2])) {
  $targetym = $GLOBALS['argv'][2];
}else{
  $targetym = isset($_GET["targetym"]) ? $_GET["targetym"] : "";
}

if(!isset($original_pid)){echo "pidセットなし強制終了";exit;}
if($original_pid == ""){echo "pid値が空なので強制終了";exit;}
if(!isset($targetym)){echo "セットなし強制終了";exit;}
if($targetym == ""){echo "値が空なので強制終了";exit;}

echo $original_pid."---".$targetym."<br>\n";
#echo "正常終了";exit;
/*
...
#DB接続
$dbh = new PDO('mysql:dbname='.DBNAME.';host='.DBHOST.';charset=utf8',DBUSER,DBPASS);
...
foreach($data as $dt) {
  ...
  $sql .= "WHERE manageperiod_targetym = '{$targetym}' ";
/*
  if($manageperiod_flag == 1):
...
#$cod = uniqid();
#acc_resultを更新
$sql = "UPDATE acc_result SET am = '{$total_copayment}' WHERE original_pid = '{$original_pid}' and targetym = '{$targetym}';";
#echo $sql;exit;
/*
