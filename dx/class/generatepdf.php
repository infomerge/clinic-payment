...
class GENERATEPDF{
  ...
  var $to;
  var $mpdf;
  var $newpage_offset = 18;
  var $newpage_offset2 = 56;
  var $newpage_offset3 = 20;
  var $dbh;
  var $targetym;
  var $mm_category;
  var $srd_start;
  ...
  function __construct() {
    ...
    #DB接続
    $this->dbh = new PDO('mysql:dbname='.DBNAME.';host=localhost;charset=utf8','root','WaV3m4DWr');
  ...
  function generate(){
    ...
    switch($this->type){
      case 1:
        #$sql .= " WHERE manageperiod_status = 2 and manageperiod_targetym = '{$this->targetym}';";
        $sql .= " WHERE re_shinryo.manageperiod_status = 2 AND patient_info.invoice_output = 0;";
        $this->outputpath = $this->targetym;
        break;
      case 2:
        $sql .= " WHERE srd >= '{$this->srd_start}' AND srd <= '{$this->srd_end}' AND patient_info.invoice_output = 0 AND patient_info.disp = 0 order by srd,category;";
        $this->outputpath = substr($this->srd_start,0,6);
        break;

      case 9:
        #$sql .= " WHERE manageperiod_status = 2 and manageperiod_targetym = '{$this->targetym}';";
        $sql .= " WHERE re_shinryo.manageperiod_targetym = {$this->targetym} AND patient_info.invoice_output = 0 order by srd,category;";
        $this->outputpath = $this->targetym;
        break;
      default:
        $sql = "";
...
#print_r($iryo_data);exit;
    ...
    foreach($data as $original_pid => $dt) {
      ...
      switch($this->type){
        case 1:
          $sql .= " WHERE appendix.manageperiod_status = 2 ";
          break;
        case 2:
...
$total_tensu = 0;
    ...
    */
    ...
    <body>";
        ...
        #顧客情報
        $html .= "<p class=\"patient-address\">〒".$patient_data['data']['postal_code']."-".$patient_data['data']['postal_code2']."<br>";
        if(isset($m_prefecture[$patient_data['data']['prefecture']]))
          $html .= $m_prefecture[$patient_data['data']['prefecture']];

        $html .= "<br>".$patient_data['data']['address1']."<br>".$patient_data['data']['address2']."</p>";
...
if($total_copayment == 0) continue;
    ...
    */
        ...
        foreach($patient_data['srd'] as $k => $v){
          ...
          #ルーティン①：日付の行の処理
            ...
            if($global_count < 2){
                $check_offset = $this->newpage_offset;
            }else{
                $check_offset = $this->newpage_offset2;
            }
    ...
    */
    ...
    if($this->mode != "debug"){
      $this->mpdf->Output("/var/www/html/".DOMAIN."/public_html/downloadpdf/".$this->outputpath."_".$this->format.".pdf",\Mpdf\Output\Destination::FILE);
    }else{
      $this->mpdf->Output();
      #$this->mpdf->Output("/var/www/html/".DOMAIN."/public_html/downloadpdf/".$this->outputpath."_".$this->format.".pdf",\Mpdf\Output\Destination::FILE);
    }
